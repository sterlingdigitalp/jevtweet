...
class Store:
    ...
    def put(self, kind: str, record_id: str, value: Any, *, replace: bool = False) -> dict:
        ...
        with self.transaction() as db:
            ...
            db.execute(
                "INSERT INTO records(kind,id,body,created_at) VALUES(?,?,?,?) ON CONFLICT(kind,id) DO UPDATE SET body=excluded.body",
                (kind, record_id, body, now().isoformat()),
            )
        return value
    ...
    def list(self, kind: str) -> list[dict]:
        with self.connect() as db:
            rows = db.execute(
                "SELECT body FROM records WHERE kind=? ORDER BY created_at,id", (kind,)
            ).fetchall()
    ...
    def cached(self, input_hash: str) -> dict | None:
        with self.connect() as db:
            row = db.execute(
                "SELECT r.body FROM cache c JOIN records r ON r.kind='judgment' AND r.id=c.judgment_id WHERE c.input_hash=?",
                (input_hash,),
            ).fetchone()
        return json.loads(row[0]) if row else None
    ...
    def reserve(self, reservation_id: str, amount: float, ceiling: float, rpm: int):
        ...
        with self.transaction() as db:
            ...
            recent = db.execute(
                "SELECT COUNT(*) FROM spending WHERE created_epoch>?", (time.time() - 60,)
            ).fetchone()[0]
            if recent >= rpm:
    ...
    def acquire_lease(self, key: str, owner: str, seconds: float = 300) -> bool:
        import time

        with self.transaction() as db:
            db.execute("DELETE FROM leases WHERE expires_epoch<?", (time.time(),))
            try:
                db.execute("INSERT INTO leases VALUES(?,?,?)", (key, owner, time.time() + seconds))
            except sqlite3.IntegrityError:
                return False
        return True

    def release_lease(self, key: str, owner: str):
        with self.connect() as db:
            db.execute("DELETE FROM leases WHERE key=? AND owner=?", (key, owner))

    def refresh_lease(self, key: str, owner: str, seconds: float = 300):
        import time

        with self.connect() as db:
            db.execute(
                "UPDATE leases SET expires_epoch=? WHERE key=? AND owner=?",
