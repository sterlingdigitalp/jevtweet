...
from jevtweet.provider import (
    API_BASE_URL, MockProvider, ProviderError, TypeSafeProvider, _retry_after, validate_response,
)
...
@pytest.fixture
def state():
    return {"candidate": {"text": "Synthetic example: a parser rejects malformed dates.", "media": {}},
            "audience": {"description": "Software builders"}, "references": [], "evidence": {"missing": []}}
...
def raw(questions):
    ...
    for key, spec in questions.items():
        if spec["type"] == "score":
            answers[key] = {"type": "score", "score": 2.5, "confidence": .7,
                            "probabilities": {"0": 0., "1": 0., "2": .5, "3": .5, "4": 0.},
                            "legend": {str(i): criterion for i, criterion in enumerate(spec["criteria"])}}
        elif spec["type"] == "choice":
            answers[key] = {"type": "choice", "choice": "assessable", "confidence": .8,
                            "probabilities": {"assessable": .9, "not_assessable": .05, "unknown": .05}}
        else:
...
def test_model_identity_mismatch_quarantines_all_answers(raw, questions, model):
    ...
    assert all(factor.score is None and factor.error == "model_mismatch" for factor in result.factors.values())
    assert "raw_response" in result.diagnostics


@pytest.mark.parametrize("value", [-.1, 4.1, float("nan"), float("inf"), "2.5", True, None])
...
@pytest.mark.parametrize("change", ["missing", "extra", "bad_sum", "negative", "nonfinite", "score_mean", "legend", "wrong_type"])
def test_score_distribution_and_legend_validation(raw, questions, change):
    ...
    if change == "missing":
        ...
        answer["probabilities"]["4"] = .2
    elif change == "negative":
        answer["probabilities"]["4"] = -.01
...
@pytest.mark.parametrize("change", ["noul_confidence", "unknown_choice", "choice_not_maximum", "choice_support", "choice_confidence"])
def test_noul_and_choice_contracts(raw, questions, change):
    if change == "noul_confidence":
        key = "missing_context"
        raw["answers"][key]["confidence"] = .9
    else:
        key = "assessability"
...
def test_missing_unknown_and_nonobject_answers_are_explicit(raw, questions):
    ...
    raw["answers"]["unexpected_id"] = {"type": "noul", "noul": .5}
    result = validate_response(raw, questions, "jev-1.13.0")
...
@pytest.mark.asyncio
@pytest.mark.parametrize("damage", ["unknown_type", "missing_score", "coerced_string", "extra_noul_confidence"])
...
@pytest.mark.parametrize("status,category,retryable", [
    (400, "request_schema", False), (401, "authentication", False), (402, "provider_credit", False),
    (403, "permission", False), (404, "model_or_endpoint_unavailable", False),
    (422, "request_schema", False), (429, "rate_limit", True), (500, "provider_server", True),
])
async def test_http_error_classification_no_internal_retries(monkeypatch, state, questions, status, category, retryable):
    requests, _ = install_transport(monkeypatch, {"error": "Private provider detail must not escape"}, status=status,
