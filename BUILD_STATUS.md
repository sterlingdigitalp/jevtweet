...
Backend suite: **366 passed, one opt-in live test skipped**; eight isolated browser tests and all four actual-backend/mock-provider integration workflows passed. The integration backend used a fresh disposable data/account directory, empty credentials and zero spending, matching normal CI. Frontend build/format and Python lint/format passed. The initial browser attempt hit sandbox loopback-bind `EPERM`; the same offline test passed with the necessary local-server permission. One upstream Starlette/AnyIO deprecation warning remains. Live tests were intentionally not run. Injected provider envelopes exercise mechanics only; they are not research evidence.

Hosted CI on the initial archive commit reproduced a browser integration timeout: the synthetic evaluation POST exceeded the 15-second UI action wait after the first three workflows passed. Local integration had passed. The test now allows up to 60 seconds for a real backend POST response, within the existing 120-second test limit; UI action limits, assertions, zero retries, backend/provider behavior and production code are unchanged. The follow-up preparation preserves the initial unapproved input freeze and verifies identical judgment request/state bytes before pinning the final tested code commit.

The final source-identity cross-check reproduced an additional import restriction gap for article IDs and wrapper pointers. A separate synthetic regression reproduced integer-valued context/quote/parent IDs being skipped. Registration now preserves article/wrapper membership and applies the same lossless integer-to-string identity normalization as source resolution (excluding booleans); both failing regressions now pass. No text, questions, scoring or request bytes change. Preparation is re-pinned after this repair; previous unapproved freezes remain preserved.
