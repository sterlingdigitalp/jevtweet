...
def development_errors(store: Store, experiment_id: str) -> dict:
    ...
    for error in experiment.get("development_errors", []):
        ...
        records.append(
            dict(
                error, candidate_text=row["text"], factors={k: row["features"].get(k) for k in SEMANTIC_NAMES}
            )
        )
    return {
...
def validate_proposals(proposals: list[dict]) -> list[dict]:
    ...
    for raw in proposals:
        ...
        if (
            ...
            or not re.fullmatch(r"[a-z][a-z0-9_]{2,63}", feature_id)
            or feature_id in seen
            or feature_id in SEMANTIC_NAMES + METADATA_NAMES
        ):
        ...
        if not all(
            isinstance(proposal[k], str) and proposal[k].strip()
            for k in ("version", "instructions", "reviewed_by", "reviewed_at", "hypothesis")
        ):
        ...
        if proposal["type"] == "score":
            ...
            if (
                ...
                or len(criteria) != 5
                or any(not isinstance(c, str) or not c.strip() or len(c) > 800 for c in criteria)
                or len(set(criteria)) != 5
            ):
...
def _safe_state(row: dict) -> dict:
    ...
    if not isinstance(state, dict):
        ...
        return {
            "candidate": {"text": row["text"], "language": row["language"], "post_type": row["post_type"]},
            "audience": {"audience_id": row.get("audience_id"), "version": row["audience_version"]},
            "prediction_cutoff": row["cutoff"],
            "synthetic": True,
        }
    allowed = {
        "candidate",
...
async def discover(
    ...
    *,
    ...
    cost_estimate_per_request_usd: float,
    max_rows: int = 200,
    ...
    if (
        not _finite(cost_limit_usd)
        or cost_limit_usd <= 0
        or not _finite(cost_estimate_per_request_usd)
        or cost_estimate_per_request_usd <= 0
    ):
        raise ValueError(
            "Discovery requires explicit positive finite total cost and conservative per-request estimates"
        )
    ...
    if next_iteration > CONFIG["discovery"]["maximum_iterations"] or (
        iteration is not None and iteration != next_iteration
    ):
    ...
    if (
        len(SEMANTIC_NAMES) + len(active) + len(proposals)
        > CONFIG["discovery"]["maximum_active_semantic_features"]
    ):
    ...
    selection_limit = min(len(selection), max(12, int(max_rows * 0.3)))
    train = train[-(max_rows - selection_limit) :]
    selection = selection[:selection_limit]
    if (
        len(train) < 12
        or len(selection) < 12
        or len({r["label"] for r in train}) < 2
        or len({r["label"] for r in selection}) < 2
    ):
        raise ValueError("Discovery needs both classes in at least 12 train and 12 selection rows")
    rows = train + selection
    allowed_keys = set(
        experiment["split_manifest"]["partitions"]["train"]
        + experiment["split_manifest"]["partitions"]["selection"]
    )
    ...
    if (
        ...
        or charged_before + required * cost_estimate_per_request_usd > cost_limit_usd + 1e-12
    ):
    ...
    result = {
        ...
        "cost_estimate_per_request_usd": cost_estimate_per_request_usd,
        "max_rows": max_rows,
        ...
        "features": [],
        "audit": [],
        "development_keys": [r["key"] for r in rows],
        "test_exposed": False,
        "production_promoted": False,
        "holdout_reuse_authorized": False,
        "limitations": [
            "Accepted features are development research only. A new untouched holdout and manual approval are required for production.",
    ...
    with store.transaction() as db:
        ...
        existing = [
            json.loads(r[0]) for r in db.execute("SELECT body FROM records WHERE kind='discovery'").fetchall()
        ]
        ...
        if len(matching) != next_iteration - 1 or any(r.get("status") == "running" for r in matching):
            raise ValueError(
                "Another discovery iteration is running or changed the experiment budget; inspect or cancel it before continuing"
            )
        db.execute(
            "INSERT INTO records(kind,id,body,created_at) VALUES('discovery',?,?,?)",
    ...
    base_loss = float(
        log_loss([r["label"] for r in selection], _sigmoid(raw_predict(base_model, selection)), labels=[0, 1])
    )
    ...
    for proposal in proposals:
        ...
        for row in rows:
            ...
            if (
                charged_before + result["charged_or_reserved_usd"] + estimate > cost_limit_usd + 1e-12
                or result["request_count"] >= max_requests
            ):
            ...
            try:
                ...
                if cost > estimate + 1e-12:
                    ...
                    result["audit"].append(
                        {
                            "event": "provider_cost_exceeded_reservation",
                            "estimate": estimate,
                            "reported": cost,
                        }
                    )
                ...
                if response.get("status") != "ok" or not _finite(value) or not 0 <= value <= maximum:
                    raise ValueError(
                        response.get("error") or "Feature answer unavailable or outside primitive range"
                    )
        ...
        candidate_loss = float(
            log_loss(
                [r["label"] for r in selection],
                _sigmoid(raw_predict(candidate_model, selection)),
                labels=[0, 1],
            )
        )
        ...
        result["audit"].append(
            {
                ...
                "reasons": feature["decision_reasons"],
                "at": now().isoformat(),
            }
        )
        _persist(store, result)
    result["status"] = "complete"
    result["active_semantic_features"] = (
        len(SEMANTIC_NAMES) + len(active) + sum(f["accepted"] for f in result["features"])
    )
    _persist(store, result)
    return result
