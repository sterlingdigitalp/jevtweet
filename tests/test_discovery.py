...
from jevtweet.evaluation import _synthetic_rows, SEMANTIC_NAMES, METADATA_NAMES
from jevtweet.discovery import discover, development_errors, validate_proposals


def proposal(name="demonstrated_result"):
    return {"feature_id": name, "version": "v1", "type": "noul", "instructions": "Does candidate.text supply a concrete demonstration?",
            "reviewed_by": "synthetic-human-review", "reviewed_at": "2025-01-01T00:00:00Z", "hypothesis": "A supplied concrete demonstration may add incremental value."}
...
def research(tmp_path):
    ...
    experiment = {"experiment_id": "experiment", "synthetic": True, "development_partitions": partitions,
                  "split_manifest": {"partitions": {**{k: [r["key"] for r in v] for k, v in partitions.items()}, "test": [r["key"] for r in rows[70:]]}},
                  "development_errors": [{"key": r["key"], "label": r["label"], "probability": .5, "absolute_error": .5, "partition": "selection"} for r in rows[40:60]],
                  "failure_examples": [{"key": rows[70]["key"], "secret_test_label": rows[70]["label"]}], "metrics": {"private_test_metric": 1}}
...
def test_incremental_feature_accepted_without_production_promotion(research):
    ...
    async def provider(state, question):
        ...
        return {"status": "ok", "execution_mode": "mock", "estimated_cost_usd": .00001,
                "value": float("demonstration" in state["candidate"]["text"])}
    result = asyncio.run(discover(store, "experiment", [proposal()], feature_provider=provider,
                                 cost_limit_usd=.1, cost_estimate_per_request_usd=.0001, max_rows=60, max_requests=100))
    assert result["status"] == "complete" and result["features"][0]["accepted"]
    assert result["features"][0]["log_loss_improvement"] > .1
...
def test_redundancy_and_failed_answers_reject(research):
    store, _ = research
    async def provider(state, question):
        return {"status": "ok", "execution_mode": "mock", "estimated_cost_usd": 0, "value": .5}
    result = asyncio.run(discover(store, "experiment", [proposal()], feature_provider=provider,
                                 cost_limit_usd=.1, cost_estimate_per_request_usd=.0001, max_rows=60))
    ...
    failed_result = asyncio.run(discover(store, "experiment", [proposal("another_result")], feature_provider=failed,
                                        cost_limit_usd=.1, cost_estimate_per_request_usd=.0001, max_rows=60))
...
def test_limits_review_and_contamination_fail_before_provider_call(research):
    ...
    async def provider(*args):
        calls.append(1)
    kwargs = dict(feature_provider=provider, cost_limit_usd=.001, cost_estimate_per_request_usd=.0001, max_rows=60)
    ...
    experiment["development_partitions"]["train"][0]["key"] = experiment["split_manifest"]["partitions"]["test"][0]
    store.put("experiment", "experiment", experiment, replace=True)
    kwargs["cost_limit_usd"] = .1
...
def test_iteration_and_active_caps(research):
    store, _ = research
    for i in range(5):
        store.put("discovery", str(i), {"discovery_id": str(i), "experiment_id": "experiment", "cost_limit_usd": .1,
                                       "charged_or_reserved_usd": 0, "features": []})
    with pytest.raises(ValueError, match="five-iteration"):
        asyncio.run(discover(store, "experiment", [proposal()], feature_provider=lambda *a: {}, cost_limit_usd=.1,
                             cost_estimate_per_request_usd=.0001, max_rows=60))
