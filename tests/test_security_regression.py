...
def client(tmp_path, monkeypatch):
    ...
    with TestClient(
        api.create_app(service=service), base_url="http://localhost:8000", raise_server_exceptions=False
    ) as test_client:
        yield test_client
...
def test_local_mutations_require_session_token_and_allowed_origin(client):
    ...
    body = {
        "content": '{"candidate_id":"csrf-fixture","text":"A synthetic CSRF fixture","synthetic":true}',
        "format": "jsonl",
    }
    ...
    for origin in (
        ...
        assert (
            client.post(
                "/api/import", json=body, headers={"X-CSRF-Token": token, "Origin": origin}
            ).status_code
            == 403
        )
    assert (
        client.post(
            "/api/import", json=body, headers={"X-CSRF-Token": token, "Sec-Fetch-Site": "cross-site"}
        ).status_code
        == 403
    )
    assert client.get("/api/session", headers={"Host": "rebinding.evil.example"}).status_code == 400
    accepted = client.post(
        "/api/import", json=body, headers={"X-CSRF-Token": token, "Origin": "http://localhost:8000"}
    )
...
def test_occurrence_and_availability_cutoffs_are_both_enforced():
    candidate = Candidate(
        candidate_id="target",
        text="A concrete repair for database deployment",
        content_available_at=T,
        provenance="hidden-winner-label.csv",
    )
    context = PredictionContext(
        prediction_cutoff=T,
        topic=EvidenceText(text="FUTURE_EVENT", occurred_at=T + timedelta(seconds=1), available_at=T),
        historical=HistoricalMetadata(
            followers=987654, observed_at=T + timedelta(seconds=1), available_at=T, provenance="hidden-label"
        ),
        references=[
            Reference(
                candidate_id="published-future",
    ...
    assert all(
        ...
        for secret in ("FUTURE_EVENT", "FUTURE_REFERENCE", "987654", "hidden-label", "hidden-winner-label")
    )
...
def test_ever_heldout_candidate_is_not_retrieved_after_later_split_reassignment(tmp_path):
    ...
    candidate = Candidate(
        candidate_id="target", text="database deployment migration test reliability", content_available_at=T
    )
    reference = Candidate(
        candidate_id="reference",
        text="database deployment safeguards catch migration errors before release",
        published_at=T - timedelta(days=4),
        content_available_at=T - timedelta(days=4),
    )
